...
- Event cards display Oracle-style evidence IDs (`HX*`, `CW*`) consistent with prompt pipeline ordering when prompt sections are available.
...
- In Task 3, each added recommendation action requires 1-5 evidence event IDs (exported as `evidence_event_ids`).
